...
class ChunksTaskTest extends ChunksTask {
  ...
  override async onCompleted(taskOutput: ChunksProcessingOutput) {
...
const create_chunks_tasks = (patchKey: PatchKey) => {
  ...
  const upperFullTask = new ChunksTaskTest('upper + chunks ').upperChunks(
    patchKey,
  )
  upperFullTask.processingParams.fakeEmpty = true
  const lowerFullTask = new ChunksTaskTest('lower + chunks').lowerChunks(
    patchKey,
  )
