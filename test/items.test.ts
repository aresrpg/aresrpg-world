...
import { ItemsTask, ItemsTaskOutput } from "../src/processing/ItemsProcessing.js"
...
import { setupTestEnv, testTaskProcessing, testTaskDelegate } from "./utils/tests_common.js"
...
const createItemsTests = (patchKey: PatchKey) => {
    const individualItems = new ItemsTaskTest('individual items').bakeIndividualChunks(patchKey)
    const mergedItems = new ItemsTaskTest('merged items').mergeIndividualChunks(patchKey)
    const pointPeakBlock = new ItemsTaskTest('items peak block').pointPeakBlock(patchKey)
...
export const itemsProcessingTests = async () => {
    ...
    const chunksTaskHandler = worldModules.taskHandlers[ChunksProcessing.handlerId] as GenericTaskHandler
    await testTaskProcessing(chunks_tasks, chunksTaskHandler)
    workerpool.nextTask
    console.log(testTaskDelegate)
