...
const createBlocksTests = (inputBatch: Vector3[]) => {
  const groundBlocks = new BlocksTaskTest('ground blocks').groundPositions(
    inputBatch,
  )
  const floorBlocks = new BlocksTaskTest('floor blocks').floorPositions(
    inputBatch,
  )
...
export const blocksProcessingTests = async () => {
  ...
  const sampledPos = samplePatch(
    patchKey,
    worldTestConf.getPatchDimensions(),
    samplingRes,
  )
  const blocksTests = createBlocksTests(sampledPos)
  const blocksTaskHandler = worldModules.taskHandlers[
    BlocksTask.handlerId
