...
import { WorldEnv, worldRootEnv } from "../config/WorldEnv.js"

import { ChunkStub } from "../datacontainers/ChunkContainer.js"
import { hashContent } from "../node/utils/chunk_node_utils.js"
import { ChunksProcessing, ChunksProcessingOutput } from "../processing/ChunksProcessing.js"
...
const get_chunk_info = async (chunkStub: ChunkStub) => {
    ...
    const hash = await hashContent(chunkStub.rawdata.buffer, 6)
...
export const create_chunks_tasks = (patchKey: PatchKey) => {
    ...
    task.onStarted = () => console.log(`TEST: lower chunks with fake empty chunks`)
    task.onCompleted = on_chunk_task_completed
    test_tasks.push(task)
    ...
    task.processingParams.skipBlobCompression = true
    task.processingParams.fakeEmpty = true
    task.onStarted = () => console.log(`TEST: upper chunks with fake empty chunks`)
    task.onCompleted = on_chunk_task_completed
    test_tasks.push(task)
...
const testChunksProcessing = async () => {
    console.log(`[TESTENV: MAINTHREAD]: chunks processing`)
    const patch_key = `-1:-1`
    const chunks_tasks = create_chunks_tasks(patch_key)
    for await (const task of chunks_tasks) {
        await task.asyncProcess()
    }
}
 ...
 * chunks polling
 */
const testChunksPolling = async (localSource: WorkerPool, world_env: WorldEnv) => {
...
const main_test_env_setup = (world_env: WorldEnv) => {
    worldRootEnv.fromStub(world_env.rawSettings)
    // console.log(Biome.instance)
    // Biome.instance.parseBiomesConfig()
}

const workerpool_test_env_setup = async (world_env: WorldEnv) => {
...
(async () => {
    ...
    // Main thread
    main_test_env_setup(world_test_config)
    await testChunksProcessing()
    // Workerpool
    workerpool_test_env_setup(world_test_config)
        .then(async workerpool => {
