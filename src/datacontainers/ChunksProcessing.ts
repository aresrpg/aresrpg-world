...
export class ChunkSetProcessor {
  ...
  async processChunksAboveGroundSurface() {
    ...
    const groundSurfaceChunks =
      await WorldComputeProxy.current.bakeSurfaceChunks(this.patchKey)
  ...
  async processChunksBelowGroundSurface() {
    ...
    const undegroundTopChunk =
      await WorldComputeProxy.current.bakeUndergroundChunk(this.patchId)
    ...
    for (let yId = undegroundTopId - 1; yId >= chunksRange.bottomId; yId--) {
      ...
      const currentChunk =
        await WorldComputeProxy.current.bakeUndergroundChunk(chunkId)
