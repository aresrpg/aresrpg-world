...
import { BlocksContainer, PatchContainer } from "./DataContainers";
...
export class BoardContainer extends PatchContainer {
    ...
    constructor(center: Vector3, radius: number) {
        ...
        this.bbox.setFromCenterAndSize(
            center.clone().floor(),
            board_dims,
        )
        this.init(this.bbox)
    ...
    getMinMax() {
        ...
        this.availablePatches.forEach(patch => {
            ...
            for (const block of blocks) {
                ...
                const dist = vect3ToVect2(block.pos)
                    .distanceTo(boardCenter)
                if (dist <= boardRadius) {
