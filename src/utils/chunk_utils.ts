...
export const concatData = (data: Uint8Array[]) => {
    ...
    const concatSize = items.reduce((sum, item) => sum + item.header.length + item.content.length, 0)
...
export const deconcatData = (concatenatedData: Uint8Array): Uint8Array[] => {
    ...
    while (offset < concatenatedData.length) {
        ...
        const contentSize = new DataView(concatenatedData.buffer).getUint32(offset, true)
...
export const chunksToCompressedBlob = async (chunks: ChunkDataContainer[]) => {
    ...
    const compressionStream = concatBlob.stream().pipeThrough(new CompressionStream('gzip'))
...
export const chunksFromCompressedBlob = async (compressedBlob: Blob) => {
    try {
        ...
        while (leftItems.length) {
            ...
            const rawdata = rawdataContent && rawdataContent.byteLength > 0 ? new Uint16Array(rawdataContent.buffer) : new Uint16Array(0)
