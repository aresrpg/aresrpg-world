...
import { PatchBoundId } from './common_types.js'
 ...
 */
export const findBoundingBox = (point: Vector2, points: Vector2[], bounds: Box2) => {
...
export const bboxContainsPointXZ = (bbox: Box3, point: Vector3) => {
    return point.x >= bbox.min.x && point.z >= bbox.min.z && point.x < bbox.max.x && point.z < bbox.max.z
}
 ...
 * @returns
 */
export const bilinearInterpolation = (p: Vector2, bounds: Box2, boundingVals: Record<PatchBoundId, Record<string, number>>) => {
    const { x, y } = p
    const { x: x1, y: y1 } = bounds.min
    const { x: x2, y: y2 } = bounds.max
    const dims = bounds.getSize(new Vector2())

    const sumComponents = (componentKey: string, values: Record<string, number>[]) => {
...
export const vectRoundToDec = (input: Vector2 | Vector3, n_pow: number) => {
    ...
    const output = input instanceof Vector3 ? new Vector3(x, y, roundToDec(input.z, n_pow)) : new Vector2(x, y)
    return output
}
