...
export const toTaskOutputStubs = (res: any) => (res instanceof Array ? res.map(item => item.toStub?.() || item) : res.toStub?.() || res)

export const parseTaskInputStubs = (...rawArgs: any) => {
    const args = rawArgs.map((rawArg: any) => {
        const arg = rawArg instanceof Array ? rawArg.map(item => parseThreeStub(item)) : parseThreeStub(rawArg)
        return arg
    })
    return args
}
...
export type ProcessingTaskHandler<ProcessingInput, ProcessingParams, ProcessingOutput> = (
    taskStub: ProcessingTaskStub<ProcessingInput, ProcessingParams>,
) => ProcessingOutput

export type AsyncProcessingTaskHandler<ProcessingInput, ProcessingParams, ProcessingOutput> = (
    taskStub: ProcessingTaskStub<ProcessingInput, ProcessingParams>,
) => ProcessingOutput | Promise<ProcessingOutput>
 ...
 */
export class ProcessingTask<ProcessingInput, ProcessingParams extends BaseProcessingParams, ProcessingOutput> {
    ...
    get awaitingProcessing() {
        return this.processingState !== ProcessingState.Done && this.processingState !== ProcessingState.Pending
    }
    ...
    process(taskHandlers: Record<TaskHandlerId, ProcessingTaskHandler<ProcessingInput, ProcessingParams, ProcessingOutput>>) {
        ...
        return taskRes ? this.onCompleted(taskRes) : taskRes // targetObj.toStub()
    }

    async asyncProcess(
        asyncTaskHandler: Record<TaskHandlerId, AsyncProcessingTaskHandler<ProcessingInput, ProcessingParams, ProcessingOutput>>,
    ) {
        this.onStarted()
    ...
    toStub() {
        ...
        const processingTaskStub: ProcessingTaskStub<ProcessingInput, ProcessingParams> = {
            processingInput,
            processingParams,
            handlerId,
            taskId,
