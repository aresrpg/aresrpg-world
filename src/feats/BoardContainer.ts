...
import { BlockType, DataContainer, GroundPatch, ProcLayer, WorldCompute, WorldConf } from '../index'

import { PseudoDistributionMap } from '../datacontainers/RandomDistributionMap'
import { BlockMode, BlocksPatch, PatchStub } from '../datacontainers/BlocksPatch'
...
export class BoardContainer extends GroundPatch {
   ...
   * @returns 
   */
  static getInitialBounds = (center: Vector3, radius: number, previousBounds?: Box2) => {
    // const previousBounds = BoardContainer.prevContainerBounds
    const defaultBounds = new Box2()
  ...
  constructor(boardCenter = new Vector3(), boardParams?: BoardInputParams, lastBoardBounds?: Box2) {
    super(BoardContainer.getInitialBounds(boardCenter, boardParams?.radius || 0, lastBoardBounds))
    const { input } = this
  ...
  overrideBlockData(block: PatchBlock) {
    ...
    blockData.type = this.isWithinBoard(block.pos) ? blockData.type : BlockType.DBG_ORANGE
  ...
  *iterBoardBlock() {
  ...
  shapeBoard() {
    ...
    DataContainer.copySourceOverTargetContainer(tempContainer, finalBoardContainer)
  ...
  // Moved to SDK
  // genStartPositions(boardContainer: BlocksPatch, otherEntities: PatchBlock[]) {
  ...
  boardSplit(boardContainer: BlocksPatch) {
    const { center } = this.input
    const boardBlocks = boardContainer.iterBlocksQuery(undefined, false);
    const dims = boardContainer.bounds.getSize(new Vector2())
    const check = (pos: Vector3) => dims.x < dims.y ? pos.z < center.z : pos.x < center.x
    for (const block of boardBlocks) {
      if (this.isWithinBoard(block.pos)) {
        block.data.type = check(block.pos) ? BlockType.DBG_ORANGE : BlockType.DBG_GREEN
  ...
  digGroundHole(holeBlock: Block, boardContainer: BlocksPatch) {
  ...
  getHolesMonoBlocks(boardContainer: BlocksPatch) {
    const holesSingleBlocks = this.getBoardEntities(boardContainer, BoardContainer.holesDistribution)
      .map(({ pos, data }) => ({ pos, data }))
    return holesSingleBlocks
  }

  getHolesAreas(boardContainer: BlocksPatch, forbiddenBlocks: Block[]) {
    const forbiddenPos = forbiddenBlocks.map(({ pos }) => asVect2(pos))
    const holesMono = this.getBoardEntities(boardContainer, BoardContainer.holesDistribution)
    ...
    holesMono.forEach(hole => {
      ...
      const holeBounds = findBoundingBox(pos, forbiddenPos, boardContainer.bounds);
  ...
  getHolesAreasBis(boardContainer: BlocksPatch, forbiddenBlocks: Block[]) {
    // prevent holes from spreading over forbidden blocks
    const isForbiddenPos = (testPos: Vector3) => !!forbiddenBlocks.find(block => block.pos.equals(testPos))
    const blocks = boardContainer.iterBlocksQuery()
    const holes: Block[] = []
    for (const block of blocks) {
      const testPos = block.pos
      if (this.isWithinBoard(testPos) && this.isGroundHole(testPos) && !isForbiddenPos(testPos)) {
        holes.push(block)
      }
    }
