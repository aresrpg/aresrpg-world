...
export class BlocksMapping {
    ...
    blockRandomization = (groundPos: Vector2, baseHeight: number, blockMapping: MappingRanges) => {
        ...
        const { amplitude } = blockMapping.data
        const bounds = {
    ...
    getBlockLevel = (rawVal: number, noSea = false) => {
        ...
        const interpolated = Utils.interpolatePoints(
            min,
            max,
            validInput,
        )
