...
export class GenChainLayer {
   ...
   */
  constructor(
    sampler: Sampler<Vector2>,
    profile: HeightProfiler,
    transition: LayerTransition = DEFAULT_TRANSITION,
  ) {
  ...
  static minValue(
    layer1: GenChainLayer,
    layer2: GenChainLayer,
    input: InputType,
  ) {
    const val1 = layer1.eval(input)
    const val2 = layer2.eval(input)
  ...
  combine(input: InputType) {
    ...
    if (rawVal <= transition.lower || !this.next) {
      ...
      const { transition } = this
      const blendFactor =
        (rawVal - transition.lower) / (transition.upper - transition.lower)
