...
import { TreeData, Vegetation } from './Vegetation'
  ...
  static updateCache(pos: Vector3) {
    ...
    for (let row = gridRange.min.x; row <= gridRange.max.x; row++) {
      for (let col = gridRange.min.y; col <= gridRange.max.y; col++) {
        const patchStart = new Vector3(
          row * BlocksPatch.patchSize,
          0,
          col * BlocksPatch.patchSize,
        )
        const patchEnd = new Vector3(
          (row + 1) * BlocksPatch.patchSize,
          0,
          (col + 1) * BlocksPatch.patchSize,
        )
    ...
    console.log(
      `${patchCount} patches generated in ${elapsedTime} ms (${perPatchAvg} ms per patch)`,
    )
  }
  ...
  *blockIterator(useLocalCoords?: boolean) {
    ...
    for (let { x } = bbox.min; x < bbox.max.x; x++) {
      for (let { z } = bbox.min; z < bbox.max.z; z++) {
        ...
        const blockType = BlockType.NONE // x % 2 ? BlockType.MUD : BlockType.SAND
  ...
  static buildPatch(patch: BlocksPatch) {
    ...
    while (!item.done) {
      ...
      if (
        !blockData.cache.genData.tree?.levelRef &&
        treeType &&
      ...
      if (blockData.cache.genData.tree?.levelRef) {
        ...
        const treeBuff = Vegetation.instance.fillTreeBuffer(
    ...
    if (!isValid)
      console.log(
        `[buildPatch] anomaly index:${blockIndex} blocks length:${blocksLength} (valid ${validBlocks})`,
      )
